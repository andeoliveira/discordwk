...
/**
 * Obtém um access token válido do Mercado Livre, renovando se necessário.
 * @returns {Promise<string>} Access token válido
 * @throws {Error} Se não for possível obter ou renovar o token
 */
export async function getValidAccessToken(): Promise<string> {
  const accessToken = await redis.get<string>('ml:access_token');
  ...
  const refreshToken = await redis.get<string>('ml:refresh_token');
